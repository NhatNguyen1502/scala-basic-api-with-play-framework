...
/**
 * Utility object containing custom Play JSON validators for common input constraints.
 */
object CustomValidators {
   ...
   *
   * @param msg The error message to display if the string is empty.
   * @return A `Reads[String]` that fails if the input string is empty.
   */
   ...
   * @param msg The error message to display if the string is too short.
   * @return A `Reads[String]` that fails if the string's length is less than `min`.
   */
   ...
   * @param msg The error message to display if the string does not match.
   * @return A `Reads[String]` that fails if the string does not match the pattern.
   */
   ...
   * @return A `Reads[Option[T]]` that validates when present, allows null or missing values.
   */
  def optionalWith[T](baseReads: Reads[T]): Reads[Option[T]] =
    Reads.optionWithNull[T](baseReads)

  /**
   * Validates a required field, and provides a custom error message when the field is missing.
   *
   * @param path The JSON path of the field to validate.
   * @param reads The base validator for the field value.
   * @param msg The error message to use if the field is missing.
   * @tparam T The type of the field.
   * @return A `Reads[T]` that applies the base validator if present, or fails with the custom message.
   */
  def requiredField[T](path: JsPath, reads: Reads[T], msg: String): Reads[T] = {
    path.read[T](reads).orElse(
